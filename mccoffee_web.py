...
# CARGA DE DATOS
df_v = pd.read_csv(db_v); df_p = pd.read_csv(db_p); df_s = pd.read_csv(db_s); df_a = pd.read_csv(db_a); df_st = pd.read_csv(db_st)
...
# --- 2. BARRA LATERAL (CONTROL DE METAS) ---
with st.sidebar:
...
with tab_v: # REGISTRO DE VENTAS
    ...
    v_v = c1.selectbox("Vendedor", l_st, key="v_1"); v_c = c2.text_input("Cliente").upper(); v_t = c3.text_input("WhatsApp")
    c4, c5, c6 = st.columns([2, 1, 1]); v_p = c4.selectbox("Producto", df_p['Cod'].tolist() if not df_p.empty else ["N/A"], key="v_2"); v_ca = c5.number_input("Cant.", min_value=1, key="v_3")
    if c6.button("➕ AÑADIR"):
    ...
    if 'car' in st.session_state and st.session_state.car:
        st.table(pd.DataFrame(st.session_state.car)[["Cant", "Nom", "Sub"]]); tv = sum(i['Sub'] for i in st.session_state.car)
...
with tab_p: # CONTROL DE PEDIDOS REVERSIBLE
    ...
    if pw == CLAVE_MAESTRA:
        ...
        with col_j1: # ENTRADA A BÓVEDA
            with st.expander("📥 SURTIR BÓVEDA CENTRAL"):
                b_p = st.selectbox("Producto Proveedor", df_p['Cod'].tolist(), key="b_p_1")
                b_n = st.number_input("Cantidad Entrada", min_value=0.1)
        ...
        with col_j2: # SALIDA A VENDEDOR
            with st.expander("🚚 RE-SURTIR VENDEDORES"):
                cv = st.selectbox("Vendedor", l_st, key="j_1"); cp = st.selectbox("Producto a Surtir", df_p['Cod'].tolist(), key="j_2")
                cn = st.number_input("Cantidad a entregar", min_value=0.1); ciu = st.text_input("Ciudad de Entrega", "CDMX")
        ...
        with st.expander("👥 GESTIÓN DE STAFF Y PRODUCTOS"):
            c_s1, c_s2 = st.columns(2); n_v = c_s1.text_input("Nuevo Vendedor")
            ...
            pc, pn, pp, pu = f1.text_input("Clave"), f2.text_input("Nombre"), f3.number_input("$"), f4.selectbox("Uni", ["KG", "PZA"])
        ...
        c_ex1.download_button("📥 Ventas", df_v.to_csv(index=False), "ventas.csv")
        c_ex2.download_button("📥 Mochilas", df_a.to_csv(index=False), "mochilas.csv")
        c_ex3.download_button("📥 Bóveda", df_s.to_csv(index=False), "boveda.csv")
        
        st.error("🚨 REINICIO"); r1, r2 = st.columns(2)
        if r1.button("BORRAR SOLO VENTAS"): pd.DataFrame(columns=["ID","Fecha","Vend","Cli","Tel","Prod","Monto","Est"]).to_csv(db_v, index=False); st.rerun()
        if r2.button("REINICIO TOTAL"): [os.remove(f) for f in [db_v, db_p, db_s, db_a, db_st] if os.path.exists(f)]; st.rerun()
